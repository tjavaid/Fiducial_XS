        ...
        def doParametersOfInterest(self):
                ...
                for iBin in range(0,self.nBin):
                    ...
                    if self.modelBuilder.out.var("r2e2muBin%d" % (iBin)):  
                        self.modelBuilder.out.var("r2e2muBin%d" % (iBin)).setRange(float(self.Range[0]), float(self.Range[1]))
                        self.modelBuilder.out.var("r2e2muBin%d" % (iBin)).setConstant(False)
                    else : 
                        self.modelBuilder.doVar("r2e2muBin%d[1, %s,%s]" % (iBin, float(self.Range[0]),float(self.Range[1])))

                    if self.modelBuilder.out.var("r4muBin%d" % (iBin)):
                        self.modelBuilder.out.var("r4muBin%d" % (iBin)).setRange(float(self.Range[0]), float(self.Range[1]))
                        self.modelBuilder.out.var("r4muBin%d" % (iBin)).setConstant(False)
                    else :
                        self.modelBuilder.doVar("r4muBin%d[1, %s,%s]" % (iBin, float(self.Range[0]),float(self.Range[1])))

                    if self.modelBuilder.out.var("r4eBin%d" % (iBin)):
                        self.modelBuilder.out.var("r4eBin%d" % (iBin)).setRange(float(self.Range[0]), float(self.Range[1]))
                        self.modelBuilder.out.var("r4eBin%d" % (iBin)).setConstant(False)
                    else :
                        self.modelBuilder.doVar("r4eBin%d[1, %s,%s]" % (iBin, float(self.Range[0]),float(self.Range[1])))
...
#             if process in [ "trueH2e2muBin0","trueH4eBin0","trueH4muBin0","trueH2e2muBin1","trueH4eBin1","trueH4muBin1","trueH2e2muBin2","trueH4eBin2","trueH4muBin2","trueH2e2muBin3","trueH4eBin3","trueH4muBin3","trueH2e2muBin4","trueH4eBin4","trueH4muBin4"]:
