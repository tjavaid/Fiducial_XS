    ...
    def doParametersOfInterest(self):
        ...
        for iBin in range(0,self.nBin):
            ...
            if self.modelBuilder.out.var("SigmaBin%d" % (iBin)):
                self.modelBuilder.out.var("SigmaBin%d" % (iBin)).setRange(float(self.SigmaRange[0]), float(self.SigmaRange[1]))
                self.modelBuilder.out.var("SigmaBin%d" % (iBin)).setConstant(False)
            else :
                self.modelBuilder.doVar("SigmaBin%d[1, %s,%s]" % (iBin, float(self.SigmaRange[0]),float(self.SigmaRange[1])))
...
class H4lZ4lInclusiveFiducialRatioV2( PhysicsModel ):
    ...
    def doParametersOfInterest(self):
        ...
        if self.modelBuilder.out.var("RatioSigmaHoZ4mu"):
            ...
            self.modelBuilder.doVar("RatioSigmaHoZ4mu[1,%s,%s]" % (self.RatioSigmaRange[0], self.RatioSigmaRange[1]))
        if self.modelBuilder.out.var("RatioSigmaHoZ2e2mu"):
            self.modelBuilder.out.var("RatioSigmaHoZ2e2mu").setRange(self.RatioSigmaRange[0], self.RatioSigmaRange[1])
        else:
...
class H4lZ4lInclusiveFiducialRatioV3( PhysicsModel ):
    ''' Model used to unfold differential distributions '''
    ...
    def doParametersOfInterest(self):
        ...
        # get values from the workspace
        fracSMH4e = self.modelBuilder.out.var("fracSMH4e").getVal()
        fracSMH4mu = self.modelBuilder.out.var("fracSMH4mu").getVal()
        fracSMZ4e = self.modelBuilder.out.var("fracSMZ4e").getVal()
        fracSMZ4mu = self.modelBuilder.out.var("fracSMZ4mu").getVal()
        ...
        if self.modelBuilder.out.var("SigmaH"):
            ...
            self.modelBuilder.doVar("SigmaH[1,%s,%s]" % (self.SigmaRange[0], self.SigmaRange[1]))
        if self.modelBuilder.out.var("HK1"):
            self.modelBuilder.out.var("HK1").setRange(0.0, 1.0/fracSMH4e)
        else:
            self.modelBuilder.doVar("HK1[1,%s,%s]" % (0.0,  1.0/fracSMH4e))
        if self.modelBuilder.out.var("HK2"):
            self.modelBuilder.out.var("HK2").setRange(0.0, (1.0-fracSMH4e)/fracSMH4mu)
        else:
            self.modelBuilder.doVar("HK2[1,%s,%s]" % (0.0, (1.0-fracSMH4e)/fracSMH4mu))
        if self.modelBuilder.out.var("RatioSigmaHoZ"):
            self.modelBuilder.out.var("RatioSigmaHoZ").setRange(self.RatioSigmaRange[0], self.RatioSigmaRange[1])
        else:
            self.modelBuilder.doVar("RatioSigmaHoZ[1,%s,%s]" % (self.RatioSigmaRange[0], self.RatioSigmaRange[1]))
...
class DifferentialFiducialCountingV3( PhysicsModel ):
    ''' Model used to unfold differential distributions for Fiducial cross-section model for both H->4l and Z->4l'''
    ...
    def setup(self):        
        for iBin in range(0,self.nBin):
            self.modelBuilder.factory_('expr::Sigma_trueZZ4eBin%d("@0*@1*@2", SigmaBin%d, fracSM4eBin%d, K1Bin%d)' % (iBin,iBin,iBin,iBin))
            self.modelBuilder.factory_('expr::Sigma_trueZZ4muBin%d("@0*(1.0-@1*@2)*@3*@4/(1.0-@1)", SigmaBin%d, fracSM4eBin%d, K1Bin%d, K2Bin%d, fracSM4muBin%d)' % (iBin,iBin,iBin,iBin,iBin,iBin) )
            self.modelBuilder.factory_('expr::Sigma_trueZZ2e2muBin%d("@0*(1.0-@1*@2)*(1.0-@3*@4/(1.0-@1))", SigmaBin%d, fracSM4eBin%d, K1Bin%d, K2Bin%d, fracSM4muBin%d)' % (iBin,iBin,iBin,iBin,iBin,iBin) )
    ...
    def getYieldScale(self,bin,process):
        ...
        for iBin in range(0,self.nBin):
            for channel in ['4e', '4mu', '2e2mu']:       
                Processes += ['trueZZ'+channel+'Bin'+str(iBin)]
        if process in Processes: return 'Sigma_'+process
        else: return 1
...
differentialFiducialCountingV3=DifferentialFiducialCountingV3()
