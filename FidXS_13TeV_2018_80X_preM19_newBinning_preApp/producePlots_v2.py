...
def plotXS(obsName, obs_bins):    
    ...
    if (obsName=="mass4l"):
        ...
        if (obsName=="pt_leadingjet_pt30_eta2p5"): offset=30.0
        if (obsName=="njets_pt30_eta2p5"): offset=999.0
    ...
    if (not opt.LUMISCALE=="1.0"):
        lumi = round(35.9*float(opt.LUMISCALE),1)
        latex2.DrawLatex(0.94, 0.94,str(lumi)+" fb^{-1} (13 TeV)")
    else:
        latex2.DrawLatex(0.94, 0.94,"35.9 fb^{-1} (13 TeV)")
...
#    c.SaveAs('plots/'+obsName+'_unfoldwith_'+datamodel+set_log+'.C')
    ...
    with open('table/table_'+opt.OBSNAME+'.py', 'w') as f:
         ...
         rbin =0
         for obsbin in range(0,len(obs_bins)-1):
             f.write('$'+str(round(data[obsbin],2))+'^{+'+str(abs(round(stat_hi[obsbin],2)))+'+'+str(abs(round(systematics_hi[obsbin],2)))+'}'+'_{-'+str(abs(round(stat_lo[obsbin],2)))+'-'+str(abs(round(systematics_lo[obsbin],2)))+'}$')
             if (obsbin == len(obs_bins)-2):
                 f.write (' \n ') 
             else:
                 f.write (' & ')
