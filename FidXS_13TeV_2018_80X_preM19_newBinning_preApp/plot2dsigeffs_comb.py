...
def parseOptions():
    ...
    parser.add_option('',   '--obsBinsReco',dest='OBSBINSRECO',    type='string',default='',   help='Bin boundaries for the diff. measurement separated by "|", e.g. as "|0|50|100|", use the defalut if empty string')
    parser.add_option('',   '--obsBinsGen',dest='OBSBINSGEN',    type='string',default='',   help='Bin boundaries for the diff. measurement separated by "|", e.g. as "|0|50|100|", use the defalut if empty string')
...
obs_bins_reco = opt.OBSBINSRECO.split("|")
if (not (obs_bins_reco[0] == '' and obs_bins_reco[len(obs_bins_reco)-1]=='')):
    print 'BINS OPTION MUST START AND END WITH A |'
obs_bins_reco.pop()
obs_bins_reco.pop(0)
if float(obs_bins_reco[len(obs_bins_reco)-1])>499:
    obs_bins_reco[len(obs_bins_reco)-1]='500'
if (opt.OBSNAME=="nJets" or opt.OBSNAME.startswith("njets")):
    obs_bins_reco[len(obs_bins_reco)-1]='4'
                        
obs_bins_gen = opt.OBSBINSGEN.split("|")
if (not (obs_bins_gen[0] == '' and obs_bins_gen[len(obs_bins_gen)-1]=='')):
    print 'BINS OPTION MUST START AND END WITH A |'
obs_bins_gen.pop()
obs_bins_gen.pop(0)
if float(obs_bins_gen[len(obs_bins_gen)-1])>499:
    obs_bins_gen[len(obs_bins_gen)-1]='500'
if (opt.OBSNAME=="nJets" or opt.OBSNAME.startswith("njets")):
    obs_bins_gen[len(obs_bins_gen)-1]='4'
...
#modelNames = ['ggH_powheg_JHUgen_125','VBF_powheg_125','WH_pythia_125','ZH_pythia_125','ttH_pythia_125']#,'SM_125','SMup_125','SMdn_125']
...
a_bins_gen = array('d',[float(obs_bins_gen[i]) for i in range(len(obs_bins_gen))])
a_bins_reco = array('d',[float(obs_bins_reco[i]) for i in range(len(obs_bins_reco))])

for model in modelNames:
    for fState in fStates:
        #eff2d = TH2D("eff2d", label, len(obs_bins_gen)-1, a_bins_gen, len(obs_bins_reco)-1, a_bins_reco)
        #folding2d = TH2D("folding2d", label, len(obs_bins_gen)-1, a_bins_gen, len(obs_bins_reco)-1, a_bins_reco)
        #eff2d4l = TH2D("eff2d4l", label, len(obs_bins_gen)-1, a_bins_gen, len(obs_bins_reco)-1, a_bins_reco)
        eff2d = TH2D("eff2d", label, len(obs_bins_gen)-1, 0, len(obs_bins_gen)-1, len(obs_bins_reco)-1, 0, len(obs_bins_reco)-1)
        folding2d = TH2D("folding2d", label, len(obs_bins_gen)-1, a_bins_gen, len(obs_bins_reco)-1, a_bins_reco)
        eff2d4l = TH2D("eff2d4l", label, len(obs_bins_gen)-1, a_bins_gen, len(obs_bins_reco)-1, a_bins_reco)
        for x in range(0,len(obs_bins_gen)-1):
            for y in range(0,len(obs_bins_reco)-1):
                eff2d.GetXaxis().SetBinLabel(x+1,str(obs_bins_gen[x])+" < "+obsName+"(gen.) < "+str(obs_bins_gen[x+1]))
                eff2d.GetYaxis().SetBinLabel(y+1,str(obs_bins_reco[y])+" < "+obsName+"(reco.) < "+str(obs_bins_reco[y+1]))   
                bin = eff2d.GetBin(x+1,y+1)
                eff2d.SetBinContent(bin,eff[model+'_'+fState+'_'+obsName+'_genbin'+str(x)+'_recobin'+str(y)])
                if (not model.startswith('SM')): eff2d.SetBinError(bin,deff[model+'_'+fState+'_'+obsName+'_genbin'+str(x)+'_recobin'+str(y)])
        ...
        c.SaveAs("plots/eff2d_"+model+"_"+obsName+"_"+fState+"_comb.png")
        c.SaveAs("plots/eff2d_"+model+"_"+obsName+"_"+fState+"_comb.pdf")
        del c
