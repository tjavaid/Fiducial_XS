...
def plotDifferentialBins(asimovDataModel, asimovPhysicalModel, obsName, fstate, observableBins):
    ...
    for fState in fStates:
        for recobin in range(nBins):
            ...
            for bin in range(nBins):
                if (bin==recobin): n_trueH_asimov[fState+"recobin"+str(recobin)] = trueH_asimov[fState+"Bin"+str(bin)+"recobin"+str(recobin)].getVal()
                else: n_trueH_otherfid_asimov[fState+"recobin"+str(recobin)] += trueH_asimov[fState+"Bin"+str(bin)+"recobin"+str(recobin)].getVal()
