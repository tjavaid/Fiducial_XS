...
##    save = save+'_58pt8fb'
    ...
    List = [
	 ...
	 'GluGluToContinToZZTo4e_13TeV_MCFM701_pythia8_slimmed',
	 'GluGluToContinToZZTo4mu_13TeV_MCFM701_pythia8_slimmed',
	 'GluGluToContinToZZTo2e2mu_13TeV_MCFM701_pythia8_slimmed',
	 #'ZZTo4L_TuneCP5_13TeV_powheg_pythia8',
#	 '2016qqZZ',
#	 'ZZTo4L_ext1_13TeV_powheg_pythia8',
	 'ZZTo4L_13TeV_powheg_pythia8_ext1_slimmed',
         #'data_13TeV_2017_duplicated_removed'
         #'ZX' #PAS
         'data_legacy2016_17Jul2018_noDuplicates_slimmed'
##	 'data_13TeV_2017'
...
##        if (Sample.startswith('Data')):   # by TJ
        ...
        #if (Sample.startswith('ZX')): 
        if (Sample.startswith('2016_') or Sample.startswith('data') ): 
            weight  = '(1.0)'
...
#            weight = '(genWeight*crossSection*dataMCWeight*pileupWeight*'+str(lumi2018)+'/'+str(sumw[Sample])+')'
            #weight = '(genWeight*crossSection*dataMCWeight*pileupWeight*'+str(lumi2016)+'/'+str(sumw[Sample])+')'
            #weight = '(genWeight*crossSection*dataMCWeight_new*pileupWeight*'+str(lumi2016)+'/'+str(sumw[Sample])+')'
            weight = '(genWeight*crossSection*dataMCWeight*pileupWeight*'+str(lumi2016)+'/'+str(sumw[Sample])+')'
...
#        if ('ZX' in Sample):   ## TJ
	#if (Sample.startswith('2016_')):
        if (Sample.startswith('2016_') or Sample.startswith('data') ): 
	    print "initially variable is ", var
	    if var=='rapidity4l': 
		var=='y4l';
	        print "the variable is", var  # yet to be fixed  FIXME
	    if (channel == '4l'):
	        cut = passedSelection+' && mass4l>0 && '+var+'>='+str(low)+' && '+var+'<'+str(high)+' && mass4l>='+str(m4llow)+' && mass4l<'+str(m4lhigh)
	    if (channel == '2e2mu'):
	        cut = passedSelection+' && mass2e2mu>0 && '+var+'>='+str(low)+' && '+var+'<'+str(high)+' && mass4l>='+str(m4llow)+' && mass4l<'+str(m4lhigh)
...
#        elif ('qqZZ' in Sample): # (Sample.startswith('ZZTo4L')):
            irr_bkg.Add(Variable[histName])
            qqzz.Add(Variable[histName])
       # elif ('MCFM' in Sample):
        elif ('GluGluToContinToZZ' in Sample): #('MCFM' in Sample):
        #elif ('ggZZ' in Sample): #('MCFM' in Sample):
            irr_bkg.Add(Variable[histName])
...
observables=['mass4l']
#observables=['mass4l','njets_pt30_eta2p5','pt_leadingjet_pt30_eta2p5','pT4l'] #,'rapidity4l','pT4l' ] #,'njets_pt30_eta4p7','pt_leadingjet_pt30_eta4p7']
#observables=['rapidity4l','pT4l' ] #,'njets_pt30_eta4p7','pt_leadingjet_pt30_eta4p7']
#observables=['pT4l' ] #,'njets_pt30_eta4p7','pt_leadingjet_pt30_eta4p7']
#observables=['mass4l'] #,'njets_pt30_eta2p5','pt_leadingjet_pt30_eta2p5']  #,'rapidity4l','njets_pt30_eta4p7','pt_leadingjet_pt30_eta4p7']
