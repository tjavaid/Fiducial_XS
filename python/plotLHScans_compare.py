...
from Utils import GetDirectory, logger
...
def parseOptions():
    ...
    combineOutputs = combineOutputs.format(year = opt.YEAR)
...
bins = {'rapidity4l': [0, 0.15, 0.3, 0.6, 0.9, 1.2, 2.5], 'pT4l': [0, 10, 20, 30, 45, 80, 120, 200, 1300], 'njets_pt30_eta2p5': [0,1,2,3,4,20], 'pTj1': [0,30,55,95,200,1300], 'mass4l': [105,160]}
...
for obsbin in obsbins:
    ...
    if(opt.UNBLIND):
        inFile = combineOutputs + "/higgsCombine"+obsName.replace(' ','_')+"_"+obsbin+"_"+opt.YEAR+unblindString+".MultiDimFit.mH125.38.root"
        f_data = TFile(inFile,"READ")
        if (f_data==0): continue
    inFile = combineOutputs + "/higgsCombine"+obsName.replace(' ','_')+"_"+obsbin+"_"+opt.YEAR+".MultiDimFit.mH125.38.root"
    ...
    if(opt.UNBLIND):
        NoSystFile = combineOutputs + "/higgsCombine"+obsName.replace(' ','_')+"_"+obsbin+"_NoSys"+"_"+opt.YEAR+unblindString+".MultiDimFit.mH125.38.root"
        fstat_data = TFile(NoSystFile,"READ")
        if (fstat_data==0): continue
    NoSystFile = combineOutputs + "/higgsCombine"+obsName.replace(' ','_')+"_"+obsbin+"_NoSys"+"_"+opt.YEAR+".MultiDimFit.mH125.38.root"
    ...
    latex2.DrawLatex(0.20,0.80, "#sigma_{fid.}^{exp} = "+str(round(bestfit_asimov,3))+" ^{+"+str(round(cl68upstat_asimov,3))+"}_{-"+str(round(cl68dnstat_asimov,3))+"} (stat.) ^{+"+str(round(sysup_asimov,3))+"}_{-"+str(round(sysdn_asimov,3))+"} (sys.)")
    if(opt.UNBLIND): latex2.DrawLatex(0.20,0.72, "#sigma_{fid.}^{obs} = "+str(round(bestfit_data,3))+" ^{+"+str(round(cl68upstat_data,3))+"}_{-"+str(round(cl68dnstat_data,3))+"} (stat.) ^{+"+str(round(sysup_data,3))+"}_{-"+str(round(sysdn_data,3))+"} (sys.)")
    ...
    OutputPath = LHScanPlots.format(year = opt.YEAR, obsName = obsName.replace(' ','_'))
    GetDirectory(OutputPath)

    c.Update()
    c.SaveAs(OutputPath+"/lhscan_compare_"+obsName+"_"+obsbin+".pdf")
    c.SaveAs(OutputPath+"/lhscan_compare_"+obsName+"_"+obsbin+".png")
